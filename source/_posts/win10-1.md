...
+ 上標單撇 : 母音字母 後 ``'`` --> á
+ 上標雙撇 : 母音字母 後 ``shift`` + ``'`` --> ä
+ 上標單撇大寫 : 切大寫後,``'``　後　母音字母  --> Á
+ 上標雙撇大寫 : 切大寫後, 母音字母 後 ``shift`` + ``'`` --> Ä
+ ``:`` --> ñ
+ ``shift``+ ``:`` --> Ñ
+ ``shift`` + ``-`` --> ?
+ ``shift`` + ``+`` --> ¿
...
+ ``shift`` + ``.`` --> :
+ ``shift`` + ``2`` --> "
+ [Type Accents with a Spanish Keyboard in Windows 10](https://www.youtube.com/watch?v=VIgvrFD9d-8&ab_channel=MaestroKaplan)
