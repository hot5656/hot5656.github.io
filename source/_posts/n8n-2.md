---
title: n8n - AI Agents, AI Automations & AI Voice Agents (1)
abbrlink: f119
...
##### google HTTP Request(YouTube) - [Youtube PlaylistItems Doc](https://developers.google.com/youtube/v3/docs/playlistItems/list) or [Youtube PlaylistItems Doc-TW](https://developers.google.com/youtube/v3/docs/playlistItems?hl=zh-tw)
``` bash
# channel 
Method: GET
...
#### ❇️google Gemini  API key - [ Google AI Studio](https://aistudio.google.com/) - ❇️[Usage](https://ai.dev/usage?tab=rate-limit), ❇️[Condition](https://ai.google.dev/gemini-api/docs/rate-limits)
``` bash
login 
--> get API key 
--> Create API key
...
#### google API key for Youtube upload
``` bash
# carete project
google Cloud --> console
  --> 點 current project
  --> create new project
  --> name : demo youtube video 
  --> create 

# create client ID
API 和 服務 --> 憑證
  --> 建立憑證 --> 建立 OAuth 用戶端 ID
    *** 第一次
    --> 設定同意畫面
      程式名稱: Yt demo
      支援郵件: hot...(my email)
      --> 外部
      聯絡資訊: kyp..(project change notify email)
      --> 建立
    --> 建立 OAuth 用戶端
      應用程式類型: 網頁應用程式
      名稱: web client demo
      已授權的重新導向 URI --> 新增 URI
        copy from "Youthbe Credention OAuth Redirect URL"
      --> 建立 (copy to "Youthbe Credention" --> sign google --> 選擇 channel)
        用戶端 ID,用戶端密碼
      --> 確定
      *** enable "YouTube Data API v3"
        ...
        目標對象(audience):
          --> 測試使用者
          --> Add users --> robert..(my email)

# Credential Setting 
OAuth Redirect URL : https://oauth.n8n.cloud... - cpoy to client ID's 已授權的重新導向 URI
...
# 解釋內容
- apikey:
放 Supabase 的 anon key 或 service role key，這是 Supabase 要求的標準 header，用來識別這是哪一個專案、並做基本授權。​
- Authorization: Bearer <api_key>:
再用同一把 key 當成 Bearer Token，Edge Function 裡如果用 Supabase client 並讀取 Authorization header，就可以拿到對應的使用者資訊或權限（視你用的是 anon key 還是有登入的 JWT）。​
- Content-Type: application/json:
告訴 Edge Function，你送過去的 body 是 JSON 格式，function 內要用 await req.json() 來解析。​
- Prefer: return=representation:
這是 Supabase PostgREST 常用的 header，很多人也會沿用在 API/Function 流程裡，意思是「請把建立好的資料直接回傳內容」，方便你在 n8n 直接拿到新增後的資料。
``` 
...
##### Supabase use for recorded communication and  RAG : [Supabase Vector Store node](https://docs.n8n.io/integrations/builtin/cluster-nodes/root-nodes/n8n-nodes-langchain.vectorstoresupabase/) --> [quickstart for setting up your vector store](https://supabase.com/docs/guides/ai/langchain?database-method=sql)
``` bash
New Project --> 
...
##### Supabse for Blog
``` bash
# add a special key for n8n workflow
...
#### [facebook developer](https://developers.facebook.com/?locale=zh_TW) WhatsApp
...
#### [Rapid](https://rapidapi.com/) - zillow(By Sabri)
``` bash
# get Aillow key
...
#### [pinecorn](https://app.pinecone.io/) - for RAG
``` bash
...
#### [groq](https://console.groq.com/) - for mamny LLM, [free limit](https://console.groq.com/settings/limits), [support Model](https://console.groq.com/docs/models)
``` bash
# API keys
...
#### [SerpAPI](https://serpapi.com/) - free 250 searches / month 
``` bash
# API Key : get API
...
# add introl video
Scenes --> Scenes1 --> Scene1 
  --> Add a new element
  --> video --> ADD 
  --> Edit --> PEXELS 
  ** 若找不到可到 Pexels, search , download 後 upload
  --> Search warrior(example) 
  --> select one
  --> save 
  --> Sacle to fit 
  --> Edit --> copy the URL to sheet "intro video"
...
Scenes --> Scenes1 --> Scene1 
  ...
  --> slect audio
  --> SAVE
  --> Edit --> copy the URL to sheet "audio List"
```
...
#### download google drive share file
``` bash
# show file by google drive & download url
share mp4 link: https://drive.google.com/file/d/1ym4IJ4pLWOlY49uPsf70XywB2GG0bbti/view
mp4 down url : https://drive.google.com/uc?id=1ym4IJ4pLWOlY49uPsf70XywB2GG0bbti&export=download
```
...
#### ❇️ [twelvedata](https://twelvedata.com/) - set key at Query Parameters
``` bash
...
#### [GNews](https://gnews.io/) - set key at Query Parameters
``` bash
...
#### [NewsAPI](https://newsapi.org/) - set key at Query Parameters
``` bash
# set at Authentication
Authentication: Generic Credential Type
Generic Auth Type: Query Auth
...
#### [Perplexity](https://www.perplexity.ai/) - [ask Perplexity API](https://www.perplexity.ai/search/perplexity-ai-support-ai-key-z-Bd5_CNBCQGSof8zDE6oWoQ#0)
``` bash
# set at Authentication
Authentication: Generic Credential Type
...
#### [Cloudinary](https://cloudinary.com/) - [Cloudinary Doc](https://cloudinary.com/documentation/image_upload_api_reference)
``` bash
# Cloud name
Home
# API key 
Settings --> API keys
# aad upload_preset
Settings --> Upload --> Add Upload Present

# set at Authentication
Authentication:Generic Credential Type
Generic Auth Type:Query Auth
Query Auth:Query Auth (Cloudinary)
  Name: api_key
  Value: <your_api_key>
...
### node
#### Chat Trigger(use publck need enable active)
``` bash
#Chat Trigger 設為 pullic
Make Chat Publicly Available: Enable 
Mode: Hosted Chat

# Chat Trigger set for page
Make Chat Publicly Available: Enable 
Mode: Embedded Chat
```
...
+ Recursive Character Text Splitter 裡，Chunk Size 決定「每塊文字的最大字元數」，Chunk Overlap 則是「相鄰兩塊之間重疊的字元數」
  ...
  + 實務上的建議數值
    ...
    2. 有人示範在 n8n 中用 Recursive Character Text Splitter 時，採用 Chunk Size 400、Overlap 100 作為範例設定，兼顧語意與長度控制。​​
  + 依你的情境調整
...
+ reference
	...
	+ [HTTP request helper for node builders](https://docs.n8n.io/integrations/creating-nodes/build/reference/http-helpers/)
  + [n8n 串接 Gemini API 教學](https://leadion.ai/blog/n8n-gemini-api-tutorial/)
+ API document
...
+ workflow example 
	...
	+ [❇️ AI Braind ai-automation-n8n](https://github.com/qwedsazxc78/ai-automation-n8n/blob/main/README.md)
...
+ practice and using
	...
	+ [❇️ OpenAI using](https://platform.openai.com/settings/organization/billing/overview)
  + [OpenAI price](https://platform.openai.com/docs/pricing)
  + [Speechify using](https://console.sws.speechify.com/billing)
+ case study
  + [I built a self-healing home lab with n8n](https://www.xda-developers.com/build-self-healing-home-lab-with-n8n/)
  + [I'm saving $40+ every month thanks to these 5 n8n automations](https://www.xda-developers.com/saving-every-month-n8n-automations/)
  + [Self-Hosted n8n Hosting from US$59.40 for 2 Years (50% off, US$2.58/Month)](https://www.ozbargain.com.au/node/932882)
  + [Build Anything With the n8n Code Node : The Secret to Advanced Automations](https://www.geeky-gadgets.com/custom-workflows-with-n8n/)
...
+ Tools
  + [❇️ VERT](https://vert.sh/) - 檔案轉換
  + [❇️ Online doc Translator ](https://www.onlinedoctranslator.com/zh-CN/) - 文字檔翻譯 or 轉換格式
+ Source
  + [❇️ pexels](https://www.pexels.com/zh-tw/) - 免費圖庫,影片